...
- 🪟 **Windows 右键菜单修复**：改用 Tauri 原生菜单（OS 层绘制），彻底消除 WebView2 Chromium 右键黑框闪烁
...
- 🌐 **划词翻译**：复制英文文本后自动弹出翻译面板，基于 LLM 提供英译中
...
- **划词翻译**：复制英文文本后自动弹出翻译面板（Ctrl+C 后 800ms），提供翻译和对话功能
...
├── src/                      # Vue 3 前端
│   ├── App.vue
│   ├── components/           # 5 个 overlay 面板（含翻译面板）
│   ├── composables/          # 7 个组合式（含翻译）
│   └── types/pet.ts          # TS 镜像
├── src-tauri/                # Rust 后端
│   └── src/
